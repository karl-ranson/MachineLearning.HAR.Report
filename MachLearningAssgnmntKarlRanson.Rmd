...
This report
Data source: Human Activity Recognition (HAR) http://groupware.les.inf.puc-rio.br/har
...
```
The raw training dataset has `r dim(pml.training)[1]` rows and `r dim(pml.training)[2]` columns. 


## Inspecting & removing useless or biased data

Looking at the validation data set, `r length(allNACols)` variables had all 'nas'. All these variables were removed from the training ('pml.training') and  validation sets.  
...
Using the Testing dataset, the oob error rate is 1- `r round(rf.cv.a,4)` = `r round(1- rf.cv.a,4)` = `r round((1- rf.cv.a)*100,4)` %. 
